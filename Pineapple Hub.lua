-- Pineapple Boombox Visualizer - COMPLETE & WORKING (2020 + caelus.lol compatible)
...
-- ==================== ALL PROPERTIES (EXACT COPY FROM YOUR ORIGINAL) ====================
...
FakeLoader.Position = UDim2.new(0.5, 0, 0.5, 0)
...
-- Input sync
Distance.Changed:Connect(function(p) if p == "Text" then _G.Distance = tonumber(Distance.Text) or 5 end end)
WaveInt.Changed:Connect(function(p) if p == "Text" then _G.WaveInt = tonumber(WaveInt.Text) or 3 end end)
Speed.Changed:Connect(function(p) if p == "Text" then _G.Speed = tonumber(Speed.Text) or 1 end end)
...
-- Visualizer (simplified but working)
local visualizers = {}
...
TextBox.MouseButton1Click:Connect(function()
	...
	for _,v in pairs(visualizers) do for _,o in pairs(v.cleanup) do pcall(o.Destroy, o) end end
	visualizers = {}
	...
	connection = RunService.RenderStepped:Connect(function()
		...
		for _, v in pairs(visualizers) do
			...
			if _G.WavesOn == "On" then
				...
				pos += Vector3.new(_G.WavesAngle == "X" and wave or 0, _G.WavesAngle == "Y" and wave or 0, _G.WavesAngle == "Z" and wave or 0)
			end
...
print("Pineapple Visualizer fully loaded! Click the launcher → Launch! → Paste audio → Visualize")
